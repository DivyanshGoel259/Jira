...
export const HeroCarousel = ({title,items}:{title:string,items:items[]}) => {
            ...
            <CarouselContent className="flex items-center pt-2 gap-5 sm:gap-20">
                {items.map((company)=>{
                    return <CarouselItem key={company.id} className="basis-1/3 lg:basis-1/6">
                            <img src={company.path} alt={company.name} width={200} height={56} className="h-9 sm:h-14 w-auto object-contain"/>
                        </CarouselItem>
                })}
            </CarouselContent>
