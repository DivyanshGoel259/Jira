...
export const CreateOrganizations = () => {
  ...
  const handleGetAllOrganizationForUser = async (selectedOrgId?:string) => {
    ...
    if(selectedOrgId){
      set({ organization:{all:data,selected:data?.filter((org:any)=>org.id==selectedOrgId)[0]}});
    } else {
                        ...
                        return <div onClick={()=>{
                          handleOrganizationClick(org);                          
                            ...
                            <div className="rounded-full">
                              <Avatar>
                                <AvatarImage className="rounded-full size-10" src={org.logo_url}/>
                                <AvatarFallback><div className="rounded-full size-10 bg-slate-800 text-white flex justify-center flex-col text-center">{org.name[0]}</div></AvatarFallback>
                              </Avatar>
                            </div>
