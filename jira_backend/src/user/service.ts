...
export const signup = async (payload: User) => {
    ...
    try {
        ...
        const createdUser = await db.one<UserType>(`
            INSERT INTO users(email,name,image_url,password) VALUES($(email),$(name),$(image_url),$(password)) RETURNING name,email,image_url,id`, { email, name, image_url, password })
...
export const signin = async (payload: Pick<UserType, "email" | "password">) => {
    ...
    try {
        ...
        const checkUser = await db.oneOrNone(`
            SELECT name,email,image_url,id FROM users WHERE email = $(email) AND password = $(password)`, { email, password })
        if (!checkUser?.id) {
...
export const gLogin = async (payload:any) => {
    ...
    try {
        ...
        if (!checkUser?.id) {
            const createdUser = await db.one<UserType>(`
                INSERT INTO users(google_user_id,email,name,image_url,password) VALUES($(sub),$(email),$(name),$(picture),$(password)) RETURNING name,email,image_url,id`, { sub, email, name, picture, password })
            const token = sign({ id: createdUser.id }, JWT_SECRET!)
            return { token, user: createdUser }
        }
        const token = sign({ id: checkUser.id }, JWT_SECRET!)
